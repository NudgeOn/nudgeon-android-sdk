# Onda Android SDK sample

`sample-app`은 로컬 `:onda` 모듈을 사용하는 최소 Android 앱입니다. 다음 공개 경로를 직접 보여 줍니다.

- `Application`에서 publishable `pk_` 키와 API host로 초기화
- `identify` / 사용자 속성 / `track` / `flush` / `reset`
- Activity Result로 Android 13+ 알림 권한 결정을 받은 뒤 Onda 상태 동기화
- 현재 FCM token 조회 후 `Onda.setPushToken`
- data-only FCM 수신 → `Onda.handleRemoteMessage` → 로컬 알림 생성
- 알림 탭 → `PendingIntent` → `Onda.handlePushOpened` → `onda-sample://push/...` 라우팅
...
onda.sdkKey=pk_your_publishable_key
onda.apiHost=http://192.168.0.10:8080
```
...
```bash
./gradlew :sample-app:installDebug \
  -PondaSdkKey=pk_your_publishable_key \
  -PondaApiHost=http://10.0.2.2:8080
```
...
1. Firebase 프로젝트에 Android 앱 `io.onda.sample`을 등록합니다.
...
4. 앱을 실행하면 시작 시 FCM token을 조회해 화면 상단 **FCM token** 칸에 전체 값을 표시하고
   `Onda.setPushToken`으로 전달합니다. 길게 눌러 복사한 뒤 콘솔/서버 테스트 발송에 사용하세요.
   **Request notification permission**으로 Android 13+ 권한을 허용해야 알림이 표시됩니다.
   **Fetch and sync current FCM token**은 수동 재조회 버튼입니다.
5. 상태 칸의 `device_id`는 서버에 등록되는 Onda 디바이스 식별자입니다. 푸시가 도착하면 Activity log에
...
샘플 서비스는 아래 문자열 data key를 받습니다. `message_id`가 없으면 Onda 메시지가 아니므로 처리하지 않습니다.
...
```json
{
  ...
  "deep_link": "onda-sample://push/orders/42",
  "data": "{\"order_id\":\"42\"}"
}
```

SDK의 기본 `OndaFirebaseMessagingService`는 data 위임만 하고 알림 UI를 만들지 않습니다. 그래서 이 앱의
...
adb shell am start -W \
  ...
  -d 'onda-sample://push/orders/42' \
  io.onda.sample
```

알림 탭으로 전달되는 Onda data 경계를 로컬에서 흉내 내려면:

```bash
adb shell am start -W \
  -n io.onda.sample/.MainActivity \
  --es message_id local-open-1 \
  --es title 'Local push open' \
  --es deep_link 'onda-sample://push/orders/42'
```
